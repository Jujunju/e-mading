    ...
    >
      <div className="container">
        ...
        <div className="collapse navbar-collapse justify-content-center" id="navbarNav" style={{ marginLeft: '-30px' }}>
          {location.pathname === `/e-mading/my-profile/${decode?.id}` || location.pathname !== `/e-mading/detail-mading/` ? (
          ...
          {!token ? (
            ...
            <div className="d-flex align-items-center gap-3" style={{ cursor: 'pointer' }} onClick={() => setIsProfileOpen(!isProfileOpen)}>
              <div className="rounded-circle overflow-hidden shadow-sm border border-2 border-success" style={{ width: '40px', height: '40px' }} data-bs-toggle="dropdown">
                <img src={`https://ui-avatars.com/api/?name=${decode?.fullName}&background=006d32&color=fff`} className="w-100 h-100" alt="profile" />
              </div>
              <div className={`profile-dropdown shadow-lg p-3 rounded-4 ${isProfileOpen ? 'dropdown-active' : ''}`}>
                <div className="text-center mb-3">
                  <img src={`https://ui-avatars.com/api/?name=${decode?.fullName}&background=006d32&color=fff`} className="rounded-circle mb-2" width="60" height="60" alt="" />
                  ...
                  <span className="badge bg-success-light text-success rounded-pill px-3 py-1 mt-1" style={{ fontSize: '10px' }}>
                    {decode?.username !== 'admin_mading' ? decode?.username : 'admin_mading'}
                  </span>
                </div>
                <hr className="text-muted opacity-25" />
                <div className="d-flex flex-column gap-1 ">
                  {location.pathname === `/e-mading/my-profile/${decode?.id}` ? (
                    <div>
                    ...
                    <div className="">
                      <Link to={`/e-mading/my-profile/${decode?.id}`} className="dropdown-item-custom rounded-3 d-flex align-items-center gap-2 py-2 px-3 text-success text-decoration-none fw-medium">
